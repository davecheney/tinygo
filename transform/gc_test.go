...
// TestGCStackSlotCounts asserts how many stack slots MakeGCStackSlots gives a
// function, which a golden file cannot do: a golden file records whatever the
// pass currently emits, so it cannot say that a slot was unnecessary.
func TestGCStackSlotCounts(t *testing.T) {
	t.Parallel()

	want := map[string]int{
		// %elem is an interior pointer whose base already has a slot.
		"loopPhiWithGEPInput": 3,
	}
