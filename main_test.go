...
func TestBuild(t *testing.T) {
	...
	t.Run("build-options", func(t *testing.T) {
		...
		// Channel queue and buffer helpers run while locks are held or
		// interrupts are disabled, so they must not allocate at -opt=0.
		t.Run("opt=0-channel-noheap-cortex-m-qemu", func(t *testing.T) {
			t.Parallel()
			opts := optionsFromTarget("cortex-m-qemu", sema)
			opts.Opt = "0"
			config, err := builder.NewConfig(&opts)
			...
			err = Build("testdata/channel.go", t.TempDir()+"/channel-noheap", config)
			if err != nil {
				w := &bytes.Buffer{}
				diagnostics.CreateDiagnostics(err).WriteTo(w, "")
				t.Fatal(w.String())
